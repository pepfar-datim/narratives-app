...
for (ou in seq_along(unique_ous)) {
...
cat("# USG Narratives")
cat('\n\n')
cat(paste0("**Operating Unit: ",unique_ous[ou]),"**")
cat('\n\n')



for (country in NROW(unique_countries))
  
  country_data<-usg_data %>% 
    dplyr::filter(country == country) %>% 
   dplyr::arrange(technical_area,support_type)

cat(paste0('## Country:',unique_countries$country[country] ))
cat('\n\n')

for (k in 1:NROW(country_data)) {
 cat(paste0("**",country_data$technical_area[k],"**"))
 cat('\n\n')
 cat(paste0(country_data$Value[k]))
cat('\n\n')
}  
}
cat('\\pagebreak') }
